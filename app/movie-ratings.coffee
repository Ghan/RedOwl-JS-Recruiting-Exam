...
class MovieRatingsResource
  ...
  getMovieRatings: (movie) -> @_movieRatings[movie] ? (throw Error 'Movie does not exist in repository')
  ...
  deleteMovieRatings: (movie) ->
    if @_movieRatings[movie]? then delete @_movieRatings[movie] else throw Error 'Movie does not exist in repository'
